...
- **SpeechSynthesis (読み上げ機能) におけるコールバック暴走・重複遷移の回避**:
  - **トラップ**: `SpeechSynthesis.cancel()` を呼び出すと、その時点で再生中だった `SpeechSynthesisUtterance` インスタンスに対して `onerror` (エラー種類: `interrupted` 等) や `onend` イベントが非同期で発火します。自動送り（`this.next()`）などの遷移処理をこれらのコールバックに記述していると、キャンセル時に不要な重複ページ遷移が連鎖し、スライドショーが暴走・スキップするバグを引き起こします。
  - **パターン**: 発話をキャンセルして次のスライドへ進む、または再生を停止する際は、必ず **`window.speechSynthesis.cancel()` を呼ぶ直前に、対象の `SpeechSynthesisUtterance` インスタンスの `onend` および `onerror` リスナーを `null` に設定して無効化**してください。
- **スライドコントロールパネルの配置とクリック透過制御**:
  - **ルール**: スライド上のコントロールボタン群（左下の音声/自動再生/フルスクリーン/発表者ビュー、右下の最初/前へ/次へ）を配置するコンテナ `.controls` は、スライド全幅に広がるように `left: 20px`, `right: 20px` に設定し、中央の空きスペースでマウスクリックを透過（スライド上のリンクやカードをクリック可能に）させるため `pointer-events: none` を適用してください。ボタンを含む子グループである `.controls-left` と `.controls-right` に対して `pointer-events: auto` を設定し、ボタンに対するインタラクションのみを復元させます。
