...
- **Rule**: Individual slide background, text, and heading overrides must be supported via HTML attributes on `hd-slide` (e.g., `type="X"` and `heading="X"`). These are styled globally inside `components.css` by overriding local slide-scoped variables. To prevent specificity issues where global attribute overrides in `components.css` accidentally override theme definitions, theme files must use selectors with matched specificity, targeting `hd-slide, hd-slide[heading], hd-slide[type]` when setting slide-level variable overrides.
