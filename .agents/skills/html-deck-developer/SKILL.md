...
- **Rule**: Standard HTML elements inside slides (such as `table`, `ul`, `ol`, `blockquote`, and `img`) must not reference global base or typography variables directly. Instead, they must map to their own element-specific customization variables (e.g., `--hd-table-font-size`, `--hd-list-font-family`, `--hd-blockquote-border-color`, `--hd-img-border-color`) declared on `:root` in `variables.css` to allow themes to configure and override them cleanly.
...
- Rule: All margins, paddings, column/split gaps, list margins, and block spacing parameters must be defined using `--hd-gap-*` spacing scale variables to maintain layout consistency.
- Rule: A complete set of directional margin, padding, and gap utility classes (`.hd-m-*`, `.hd-p-*`, `.hd-gap-*`) are available for levels `0` to `6` to customize spacing dynamically (e.g. `.hd-mt-3`, `.hd-px-4`, `.hd-gap-5`).
...
- **Trap (Active/Dynamic Variable Leakage)**: Declaring global intermediate variables (e.g., `--hd-color`) on `:root` to represent the "active" variant and mapping component variables to them (e.g., `--hd-card-background-color: var(--hd-color-muted)`) causes container variant settings to inherit unexpectedly. For example, a default card nested inside a main variant slide will inherit the slide's overridden active variables, causing the card to render as a main variant card.
- **Rule**: Avoid global intermediate active variables on `:root` for components that require independent variant styling. Map component properties directly to the base palette variables on `:root` (e.g., `--hd-card-background-color: var(--hd-base-color-muted)`), and override them inside the component's internal Shadow DOM style sheet using specific attribute selectors (e.g., `:host([variant="main"])` in `hd-card.js`).
...
### 25. Slotted Header Wrapper and Presence Toggling
- **Trap (Shadow DOM :host(:has(...)) Limitations)**: Trying to toggle visibility of a container header wrapper using CSS selectors like `:host(:has([slot="heading"]))` can fail or exhibit unexpected behavior in modern browsers because `:host()` has limited ability to query deep child elements slot assignments.
- **Rule**: Avoid wrapping `<slot name="heading"></slot>` inside an additional header container `div` if the goal is to style the heading with backgrounds or borders only when it is present. Instead, place `<slot name="heading"></slot>` directly under `:host` and style the slotted element using `::slotted([slot="heading"])`. When no element is slotted, the slot takes up `0` height and adds no borders or padding automatically, making presence toggling simple and fully declarative.
